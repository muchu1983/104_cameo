    ...
    def parseNewsPageThenCreateNewsJson(self, uselessArg1=None):
        ...
        for strNewsHtmlFilePath in lstStrNewsHtmlFilePath:
            ...
            with open(strNewsHtmlFilePath, "r") as newsHtmlFile:
                ...
                strCssNotFilter = ":not(span.entry-author):not(span.entry-date):not(a[rel=tag]):not(div.digit):not(span.share-count):not(h2.entry-title):not(a[rel=prev]):not(a[rel=next]):not(script):not(iframe)"
