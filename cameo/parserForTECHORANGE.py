    ...
    def parseNewsPageThenCreateNewsJson(self, uselessArg1=None):
        ...
        for strNewsHtmlFilePath in lstStrNewsHtmlFilePath:
            ...
            with open(strNewsHtmlFilePath, "r") as newsHtmlFile:
                ...
                #strContent
                strCssNotFilter = ":not(script):not(h2.entry-title):not(a[rel=prev]):not(a[rel=next]):not(div.digit):not(iframe):not(span.entry-author):not(span.entry-date):not(a[rel=tag])"
